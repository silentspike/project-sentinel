...
- `activate` validates the raw provision-receipt digest, manifest digest, Git
  ...
  the readiness authority. After the target-start mutation, the complete
  prevalidated topology is invocation-owned for rollback: the target is stopped
  first, then every canonical unit in reverse order, with bounded readback.
...
wait for the child before activation rollback or termination.
