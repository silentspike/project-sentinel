...
//!
//! **Scope (Maintainer-Entscheid): NUR `agent_live`.** Das Frontend hat aktuell nur Handler fuer
//! `hello`+`agent_live` (`console/src/stores/console.ts`); `room_live`/`kpi`/`tasks` haben keinen
//! Konsumenten und gehoeren an die 9-View-Migration (#433) — daher hier kein Push, keine Extraktion.
//!
...
/// Reine Raum-/KPI-/Task-Events: sie aendern das `agent_live`-Read-Model NICHT und loesen daher
/// keinen Push aus. Alles **nicht** hier Gelistete (Agent-Events, `config_applied`, sowie unbekannte/
/// neue Typen) ist fail-safe push-relevant — ein neuer Event-Typ darf die View nicht still stale machen.
...
/// Ob ein Event-Typ einen `agent_live`-Re-Read/Push ausloest.
///
/// Agent-relevante Events (Spawn/Despawn/Action/Transit/Status/Bio/Schichtwechsel) und `config_applied`
/// (Resync) → `true`. Reine Raum-/KPI-/Task-Events → `false`. **Default/unbekannt → fail-safe `true`**
/// (lieber einmal zu viel re-read als eine still stale Agent-View).
pub(crate) fn classify(event_type: &str) -> bool {
    !NON_AGENT_EVENT_TYPES.contains(&event_type)
}
...
pub async fn run_event_subscriber(state: AppState) {
    ...
    loop {
        match subscribe_and_pump(&state).await {
            ...
            Err(e) => tracing::warn!(error = %e, "event subscriber error, reconnecting with backoff"),
        }
    ...
    loop {
        tokio::select! {
            maybe = messages.next() => match maybe {
                Some(Ok(msg)) => {
                    if classify(event_type_from_subject(msg.subject.as_str())) {
                        dirty = true;
                    }
                }
...
fn push_agent_live(state: &AppState) {
    match crate::projection::agents_rows(&state.config.projection_db) {
        Ok(rows) => match crate::codec::encode_frame("agent_live", &serde_json::json!({ "agents": rows })) {
            Ok(frame) => {
...
mod tests {
    ...
    fn classify_covers_full_event_type_list() {
        // Jeder bekannte Typ ist eindeutig push-relevant (true) oder bewusst nicht (NON_AGENT-Liste).
        for &et in ALL_KNOWN_EVENT_TYPES {
            let expected = !NON_AGENT_EVENT_TYPES.contains(&et);
    ...
    fn agent_relevant_events_trigger_push() {
        for &et in AGENT_RELEVANT {
            assert!(classify(et), "{et} muss einen agent_live-Push ausloesen");
        }
    }

    #[test]
    fn pure_room_kpi_task_events_do_not_push() {
        for &et in NON_AGENT_EVENT_TYPES {
            assert!(!classify(et), "{et} darf agent_live nicht ausloesen");
        }
    }

    #[test]
    fn config_applied_triggers_resync_push() {
        assert!(classify("config_applied"), "config_applied muss einen Resync-Push ausloesen");
    }
    ...
    fn unknown_event_type_is_push_relevant() {
        ...
        assert!(classify("some_future_event_v2"));
        assert!(classify(""));
    }
    ...
    fn event_type_parsed_from_subject() {
        ...
        assert_eq!(event_type_from_subject("sentinel.events.config_applied.runtime"), "config_applied");
