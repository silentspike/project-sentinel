...
fn main() {
    ...
    println!("Magic: {:02x} {:02x} {:02x} {:02x}", probe_bytes[0], probe_bytes[1], probe_bytes[2], probe_bytes[3]);
    println!("Pointer address: {:p}", probe_bytes.as_ptr());
    println!("Pointer alignment: {} (mod 8 = {})",
    ...
    println!("  Aligned ptr: {:p}, mod 8 = {}", aligned.as_ptr(), aligned.as_ptr() as usize % 8);
    match object::File::parse(&aligned[..]) {
        Ok(file) => {
            use object::Object;
            println!("  object::File::parse: OK! Architecture: {:?}", file.architecture());
            use object::ObjectSection;
            for section in file.sections() {
                println!("    [{}] {:?} size={}", section.index().0, section.name().unwrap_or("?"), section.size());
            }
        }
        Err(e) => println!("  object::File::parse: FAILED: {}", e),
    }
