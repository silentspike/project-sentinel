...
| 173 | 2026-03-30 16:00 | X19 Dashboard Control Modul | PASS | `dashboard/public/js/control.js` bereinigt: doppelte `authHeaders()`-Deklaration entfernt. Browser-Verifikation lokal und auf `http://10.0.0.240:8000`: Control-Tab rendert wieder mit `8` `.control-section`-Sektionen; Texte wie `Synthesis Rate` und `Active Patterns` sind im echten DOM sichtbar. |
| 174 | 2026-03-30 16:02 | X20 API-CP Restore-Heal | PASS | Neuer Gateway-Build deployed. `restore()` rekonstruiert `Count`, `TopHash`, `TopContent` und `Confidence` jetzt aus `response_hashes`. Live-Evidence auf `10.0.0.240`: `traffic-stats.apicp.promoted_patterns=1`, `suggestions=1`; daemonseitiger Snapshot fuer Agent `12` / Fingerprint `H9|E6|B2|S2|C4|SN3|R:buero-dev-1|P:0|CH:0|HR:0|T:18|TMP:0|PE:E|IM:0` zeigt nach Restart `count=50`, `confidence=1.0`, `promoted=true`. |
| 175 | 2026-03-30 22:22 | X21 Cockpit Legacy Schema | PASS | `dashboard/src/db.ts` und `dashboard/src/routes/events.ts` nutzen jetzt einen spaltenbewussten `eventRowSelectColumns()`-Pfad. Regressionstests fuer ältere `events`-Schemas ohne `compensation_type` sind lokal grün; live liefert `GET http://127.0.0.1:8000/api/cockpit` auf `10.0.0.240` wieder `200 OK` statt `SQLiteError: no such column: compensation_type`. |
| 176 | 2026-03-30 22:25 | X22 MITM Streaming + Raw Blocks | PASS | Der externe MITM-Pfad `/v1/messages` versteht jetzt `stream=true`, hält eingehende Anthropic-Content-Blocks roh und forwardet direkt über `anthropic-direct`. Live-Test: `ANTHROPIC_BASE_URL=http://10.0.0.240:8080 claude -p 'Antworte exakt mit HALLO.' --output-format json` endet erfolgreich mit `HALLO.`; Claude-Code meldet `cache_creation.ephemeral_1h_input_tokens=16356` und das Gateway-Journal zeigt `pipeline stream completed` mit `provider=\"anthropic-direct\"`. |
| 177 | 2026-03-30 22:26 | X23 Interner vs. externer Pfad | PASS | Der interne Service-Vertrag ist auf `/internal/llm` getrennt und live aktiv. `POST http://10.0.0.240:8080/internal/llm` mit einem synthese-faehigen Request fuer `agent_id=5` liefert `provider=\"synthesis\"`, `model=\"sentinel-synth-v1\"`, korrekte Actions und Ziel `toilette-eg-herren`. `traffic-stats` exponiert zusaetzlich `internal_primary_provider=\"claude-code\"` und `external_mitm_provider=\"anthropic-direct\"`, im Control-Tab browserseitig sichtbar als `Internal Provider` und `External MITM Provider`. |
| 178 | 2026-03-30 22:26 | X24 Redaction + MITM Smoke | PASS | Redaction-Audit: `PassthroughHeaders` werden nicht in JSON serialisiert (`TestLLMRequestJSONDoesNotMarshalPassthroughHeaders`), und Journal-Grep auf `Authorization|x-api-key|anthropic-version|anthropic-beta` zeigt nach Live-Calls keine Auth-Leaks. Der reproduzierbare Smoke-Test `scripts/mitm-smoke.sh` ist lokal grün und weist `claude -p -> Gateway -> Fake Anthropic API` mit `provider=\"anthropic-direct\"` nach. |
