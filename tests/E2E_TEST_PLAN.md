...
- **Aktion:** `GET http://192.0.2.240:8080/health`
...
- **Aktion:** `GET http://192.0.2.240:8081/health`
- **Erwartung:** HTTP 200, JSON `{"status":"ok"}`
- **Fail-Kriterium:** Status != 200

### T1.12 — NATS Bridge Ready [P0] [HTTP]
- **Aktion:** `GET http://192.0.2.240:8083/ready`
...
- **Aktion:** `GET http://192.0.2.240:8000/api/health`
- **Erwartung:** HTTP 200, JSON `{"status":"ok","uptime":N,"projection_lag":N}`
- **Fail-Kriterium:** Status != 200 oder fehlende Felder

### T1.14 — Sentinel Projection Worker laeuft [P0] [SSH]
- **Aktion:** `ssh ubuntu@192.0.2.240 "systemctl is-active sentinel-projection"`
- **Erwartung:** Output "active"
- **Fail-Kriterium:** "inactive" oder "failed"

### T1.15 — NATS Monitoring Port [P1] [HTTP]
- **Aktion:** `GET http://192.0.2.240:8222/varz`
...
- **Aktion:** Playwright WebSocket an `ws://192.0.2.240:8000/ws` verbinden
...
- **Aktion:** `GET http://192.0.2.240:8080/metrics`
...
- **Aktion:** `PATCH http://192.0.2.240:8081/control/config` mit `{"temperature": 0.5}`
...
- **Aktion:** `POST http://192.0.2.240:8081/control/provider` mit `{"provider":"ollama"}`
...
- **Aktion:** `GET http://192.0.2.240:8081/control/guardrails`
...
- **Aktion:** `POST http://192.0.2.240:8080/v1/chat/completions` mit Test-Payload
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "/opt/sentinel/bin/nats-server --version"`
- **Erwartung:** Version 2.12.4 oder hoeher
- **Pruefung:** Versionsnummer extrahieren

### T12.2 — SENTINEL_EVENTS Stream existiert [P0] [NATS]
- **Aktion:** `ssh ubuntu@192.0.2.240 "nats stream info SENTINEL_EVENTS"`
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "nats stream info SENTINEL_JUDGE"`
- **Erwartung:** Stream konfiguriert, 30-day retention, 100MB max
- **Pruefung:** Retention, MaxBytes korrekt

### T12.4 — judge-heuristic Consumer existiert [P0] [NATS]
- **Aktion:** `ssh ubuntu@192.0.2.240 "nats consumer info SENTINEL_EVENTS judge-heuristic"`
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "nats consumer info SENTINEL_EVENTS judge-batch"`
- **Erwartung:** Durable Pull Consumer aktiv
- **Pruefung:** Analog T12.4

### T12.6 — Bridge publiziert Events [P0] [SSH]
- **Aktion:** `ssh ubuntu@192.0.2.240 "nats stream info SENTINEL_EVENTS"` — Messages-Count pruefen
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "nats stream subjects SENTINEL_EVENTS"`
...
- **Aktion:** NATS Monitoring `GET http://192.0.2.240:8222/jsz`
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "systemctl show sentinel-nats-bridge --property=NRestarts"`
...
- **Aktion:** `GET http://192.0.2.240:8082/metrics`
...
- **Aktion:** `POST http://192.0.2.240:8082/api/v1/analyze` mit Test-Payload
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "systemctl show sentinel-judge --property=NRestarts"`
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "systemctl is-active sentinel-daemon"`
- **Erwartung:** "active"
- **Pruefung:** Output === "active"

### T14.2 — Daemon Uptime > 1h [P0] [SSH]
- **Aktion:** `ssh ubuntu@192.0.2.240 "systemctl show sentinel-daemon --property=ActiveEnterTimestamp"`
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "sqlite3 /opt/sentinel/data/events.db 'SELECT COUNT(*) FROM events'"`
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "ps -o rss= -p $(pgrep sentinel-daemon)"`
- **Erwartung:** < 200 MB (typisch ~62 MB nach 21h)
- **Pruefung:** RSS < 200000 KB

### T14.6 — Daemon kein Crash-Loop [P0] [SSH]
- **Aktion:** `ssh ubuntu@192.0.2.240 "systemctl show sentinel-daemon --property=NRestarts"`
- **Erwartung:** NRestarts == 0
- **Pruefung:** Kein Restart

### T14.7 — 54 Agent-Configs geladen [P1] [SSH]
- **Aktion:** `ssh ubuntu@192.0.2.240 "ls /opt/sentinel/config/agents/*.toml | wc -l"`
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "systemctl list-timers | grep nightrun"`
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "python3 -c \"import sqlite3; c=sqlite3.connect('/opt/sentinel/data/projection.db'); print([r[0] for r in c.execute('SELECT name FROM sqlite_master WHERE type=\\'table\\'')]); c.close()\""`
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "systemctl show sentinel-projection --property=NRestarts"`
- **Erwartung:** NRestarts == 0
- **Pruefung:** Kein Crash-Loop
...
- **Aktion:** `ssh ubuntu@192.0.2.240 "python3 -c \"import sqlite3; c=sqlite3.connect('/opt/sentinel/data/events.db'); print(c.execute('SELECT COUNT(*) FROM outbox WHERE status=\\'pending\\'').fetchone()[0]); c.close()\""`
