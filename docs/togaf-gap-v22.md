...
| Registry and workload selection (#411/#472) | 🟡 | `NanoRuntimeRegistry` selects explicit Agent TOML runtime keys with an explicit `bwrap-landlock` fallback. The source candidate registers ECS-native, bwrap+Landlock, and optional WASM adapters and routes productive lifecycle classes through a private typed owner. Health observes exact instance-fenced adapter state and accepts `Degraded` only for an intentionally `Suspended` runtime. Config Apply has one versioned SQLite decision saga plus a replayable filesystem participant and startup-fenced rollback/forward recovery. The compile-fail visibility test is the ownership barrier; the AST inventory is supplementary drift detection only. Default bwrap snapshots reproduce a fresh runtime from the bound workload specification without claiming process-state capture. WASM restore binds completed results without replaying stored input. The optional bwrap CAS manifest remains default-off for #548. Exact-head live evidence for the three-adapter production phase is pending, and microVM is deliberately rejected rather than overclaimed. |
| microVM adapter + local live-migration (#417 / #413) | 🟡 | `crates/sentinel-microvm/src/{microvm,firecracker}.rs` implements an adapter and has historical Firecracker/KVM conformance evidence, but the repository lacks the packaged guest launcher, workload-bound readiness attestation, and durable launch ownership/restart reconcile contract required for productive selection. The #472 source candidate therefore rejects `runtime = "microvm"` fail-closed. SINGLE_NODE productization is tracked by #775 under #650 after #687; #553/#554 retain cross-node and deep-migration ownership. #413 separately adds local ECS-native live migration. |
