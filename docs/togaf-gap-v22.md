...
| Registry and workload selection (#411/#472) | 🟡 | `NanoRuntimeRegistry` selects explicit Agent TOML runtime keys with an explicit `bwrap-landlock` fallback. The source candidate registers ECS-native, bwrap+Landlock, and optional WASM adapters and routes productive lifecycle classes through a private typed owner. Health now observes exact instance-fenced adapter state. The compile-fail visibility test is the ownership barrier; the AST inventory is supplementary drift detection only. Default bwrap snapshots reproduce a fresh runtime from the bound workload specification without claiming process-state capture. WASM restore binds completed results without replaying stored input. The optional bwrap CAS manifest remains default-off for #548. Exact-head live evidence is pending, and microVM is deliberately rejected rather than overclaimed. |
| microVM adapter + local live-migration (#417 / #413) | 🟡 | `crates/sentinel-microvm/src/{microvm,firecracker}.rs` implements an adapter and has historical Firecracker/KVM conformance evidence, but the repository lacks the packaged guest launcher, workload-bound readiness attestation, and durable launch ownership/restart reconcile contract required for productive selection. The #472 source candidate therefore rejects `runtime = "microvm"` fail-closed and leaves AC-1/AC-4 blocked. #413 separately adds local ECS-native live migration; cross-node migration remains out of scope. |
