...
| Registry and workload selection (#411/#472) | 🟡 | `NanoRuntimeRegistry` selects explicit Agent TOML runtime keys with an explicit `bwrap-landlock` fallback. The reviewed source candidate registers all four adapters and routes startup, restart, staged config replacement, shift transition, restore replacement, operator despawn, shutdown, pause/resume, restart re-suspend, and control-plane suspension through the selected adapter. Tests cover fallback, explicit ECS-native and WASM selection, invalid-key rejection, incarnation fencing, retryable cleanup, transactional bwrap spawn failure at each side-effect boundary, cgroup/eBPF stop ordering, pre-commit world-restore compensation, stop-before-publish config changes, registry-owned handle recovery, and fail-closed control without false logical success. Productive bwrap CAS world snapshot/restore remains default-off until #548 provides the durable ownership and GC-safe pin contract; #472 makes no production bwrap restore claim. Exact-head Deploy-VM evidence is still required before this row can become green. |
