...
| Registry and workload selection (#411/#472) | 🟡 | `NanoRuntimeRegistry` selects explicit Agent TOML runtime keys with an explicit `bwrap-landlock` fallback. The reviewed source candidate registers all four adapters and routes startup, restart, config apply, shift transition, restore replacement, operator despawn, and shutdown through the selected adapter. Tests cover fallback, explicit ECS-native and WASM selection, invalid-key rejection, incarnation fencing, retryable cleanup, exact runtime snapshot restore, and runtime-aware reconcile. Exact-head Deploy-VM evidence is still required before this row can become green. |
