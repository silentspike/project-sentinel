...
| Registry and workload selection (#411/#472) | ✅ | `NanoRuntimeRegistry` selects explicit Agent TOML runtime keys with an explicit `bwrap-landlock` fallback. The production daemon registers all four adapters and routes startup, restart, config apply, shift transition, restore, and shutdown through the selected adapter; tests cover fallback, explicit ECS-native selection, invalid-key rejection, and workload-scoped stop. |
| microVM adapter + local live-migration (#417 / #413) | ✅ | `crates/sentinel-microvm/src/{microvm,firecracker}.rs` implements the contract and passes the conformance harness (live-verified on the Deploy-VM, real Firecracker/KVM). The adapter is registered in the productive daemon path by #472. #413 separately adds local ECS-native live migration (`POST /operator/migrate` -> `MigrationCompleted` event, live-verified); cross-node migration remains out of scope. |
