...
| NanoRuntime trait + ECS reference (#408/#698) | ✅ | `crates/sentinel-common/src/nano_runtime.rs` defines `spawn`/idempotent `stop`/`exec`/`snapshot`/`restore`/`migrate`/`health`/`isolate` plus typed runtime-owned suspend/resume `control`; `crates/sentinel-runtime/src/nano.rs` proves the ECS-native reference, workload-scoped stop, and incarnation-fenced control through the shared conformance harness |
| WASM/Wasmtime adapter (#409)  | ✅ | `crates/sentinel-wasm/src/nano.rs` snapshots the declarative workload/tool binding and an already-bound execution result, never re-executes stored input during restore, and does not claim a bitwise Wasmtime Store dump. External-effect retry requires a separate durable idempotency key and receipt. |
...
| Registry and workload selection (#411/#472) | 🟡 | `NanoRuntimeRegistry` selects explicit Agent TOML runtime keys with an explicit `bwrap-landlock` fallback. The reviewed source candidate registers all four adapters and routes startup, restart, staged config replacement, shift transition, restore replacement, operator despawn, shutdown, pause/resume, restart re-suspend, and control-plane suspension through a typed lifecycle owner in a private orchestrator submodule. A compile-fail visibility test proves external production code cannot import either that owner or the daemon's live registry; an AST inventory remains supplementary drift detection only. Functional tests cover fallback, explicit ECS-native and WASM selection, invalid-key rejection, incarnation fencing, retryable cleanup, transactional bwrap spawn failure at each side-effect boundary, cgroup/eBPF stop ordering, pre-commit world-restore compensation, stop-before-publish config changes, crash-persistent non-serving recovery, registry-owned handle recovery, and fail-closed control without false logical success. Default bwrap snapshots reproduce a fresh runtime from the bound workload specification and therefore remain usable by World Snapshot, Restore, Time Machine, and Config Apply without claiming process-state capture. WASM restore binds declarative runtime state and completed results without replaying stored input. Only the optional bwrap CAS filesystem-manifest extension remains default-off until #548 provides the durable ownership and GC-safe pin contract. Exact-head Deploy-VM evidence is still required before this row can become green. |
| microVM adapter + local live-migration (#417 / #413) | 🟡 | `crates/sentinel-microvm/src/{microvm,firecracker}.rs` implements the adapter contract and has historical real Firecracker/KVM conformance evidence. The #472 source candidate registers it in the productive daemon path and now passes the complete workload identity and command through the versioned [guest workload contract](microvm-guest-workload-contract.md). A compatible rootfs containing the declared guest launcher and exact-head daemon spawn/snapshot/restore evidence are still required. #413 separately adds local ECS-native live migration (`POST /operator/migrate` -> `MigrationCompleted` event, live-verified); cross-node migration remains out of scope. |
...
| Tracking epic (#397)          | 🟡 | #397 remains the Cluster 12 tracking epic; #407-#411 deliver the contract chain, #413/#417 add local live migration and the microVM adapter, and #472 integrates per-workload selection into the productive daemon path. Cross-node live migration remains out of scope. |
