...
patches, source replacements, or fork URLs, so they are not registry rows. They
remain dependency-ownership and upgrade inputs for #705 and #656.
...
Ordinary official-upstream Git dependencies are reported as inventory but do not
silently become patch/fork entries. A future fork must use a recognized Cargo
override mechanism and pass this registry gate.
