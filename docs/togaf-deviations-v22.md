...
| Realized | **Production runtime selection realized (#472):** the daemon constructs one `NanoRuntimeRegistry` with the four real adapters and an explicit `bwrap-landlock` fallback. Every productive agent start, restart, config apply, shift transition, restore, and shutdown now uses the adapter selected by `[runtime].nano_runtime`; the selected adapter owns its process and resource lifecycle, while daemon cgroup/eBPF state remains observational. `crates/sentinel-microvm` supplies the Firecracker/KVM adapter (#417), and the shared idempotent per-workload stop contract (#698) prevents cross-workload cleanup. Snapshot semantics remain runtime-specific; local ECS-native migration (#413) remains separately available through `POST /operator/migrate`, while cross-node migration is out of scope. |
