...
| Runtime contract | A compliant runtime implements seven operations: `spawn`, `exec`, `snapshot`, `restore`, `migrate`, `health`, and `isolate`. Initial runtime families are `ecs-native`, `wasm-wasmtime`, `bwrap-landlock`, and `microvm` (realized — see Realized row). |
...
| Realized | **microVM realisiert (#417):** `crates/sentinel-microvm` implementiert den Sieben-Operationen-`NanoRuntime`-Vertrag ueber Firecracker (KVM) und besteht die Conformance-Harness (#408). Snapshot-Semantik `MicrovmMemory` (stabile Metadaten + deterministische Firecracker mem/state-Pfade je `workload_id`). Damit ist der erste "Revisit when"-Trigger erfuellt. Zusaetzlich: lokale Live-Migration als beobachtbare First-Class-Operation geliefert (#413, ECS-native, `POST /operator/migrate` + `MigrationCompleted`-Event). Cross-node-Migration bleibt offen (Multi-Node-gated). |
| Files | `docs/togaf-deviations-v22.md`, `docs/togaf-gap-v22.md`, `crates/sentinel-common/src/nano_runtime.rs`, `crates/sentinel-runtime/`, `crates/sentinel-wasm/`, `crates/sentinel-sandbox/`, `crates/sentinel-microvm/`, #397, #407, #408, #409, #410, #411, #413, #417 |
