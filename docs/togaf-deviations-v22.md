...
| Profile (6 points) | (1) identical CPU model OR identically masked CPU features · (2) identical binary / Rust version (`rust-toolchain.toml` = 1.95.0) / compiler flags · (3) no `fast-math`, no unplanned FMA contraction · (4) same `libm`/`std` · (5) fixed rounding/subnormal/FMA policy · (6) deterministic iteration/DB order. |
...
| Enforcement | CI `#[test] determinism_two_runs_identical_hash` in `crates/sentinel-ecs/src/hash.rs` (two identical bio+ECS tick sequences → identical STRICT+CORE hash, intra-run gate) · `scripts/check-determinism-profile.sh` (toolchain version + no fast-math/FMA/`target-feature` flags) · a CI step asserting the **effective** `rustc --version` == `1.95.0` in every Rust job (the `dtolnay/rust-toolchain` action is pinned to a commit tagged `# stable`, not a literal channel — so the action ref alone is not proof; the jobs also pin `toolchain: "1.95.0"`) · `deploy/deploy-preflight.sh` sha256 binary parity on all nodes. |
