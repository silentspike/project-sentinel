...
| Runtime contract | A compliant runtime implements eight operations: `spawn`, idempotent per-workload `stop`, `exec`, `snapshot`, `restore`, `migrate`, `health`, and `isolate`. Initial runtime families are `ecs-native`, `wasm-wasmtime`, `bwrap-landlock`, and `microvm`; all four are registered in the production daemon and selected per Agent TOML through the shared registry (#472). |
...
| Revisit when | cross-node migration becomes a product requirement; a new runtime family is proposed; or production evidence shows the eight-operation contract or explicit fallback policy is too weak or too broad for real workloads |
