...
| Enforcement | CI `#[test] determinism_two_runs_identical_hash` in `crates/sentinel-ecs/src/hash.rs` (two identical bio+ECS tick sequences → identical STRICT+CORE hash, intra-run gate) · `scripts/check-determinism-profile.sh` (toolchain version + no fast-math/FMA/`target-feature` flags) · a CI step asserting the **effective** `rustc --version` == `1.97.1` in every Rust job (the `dtolnay/rust-toolchain` action is pinned to a commit tagged `# stable`, not a literal channel — so the action ref alone is not proof; the jobs also pin `toolchain: "1.97.1"`) · `deploy/deploy-preflight.sh` sha256 binary parity on all nodes. |
