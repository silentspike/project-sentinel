...
| Realized | **Production-path implementation (#472, live verification pending):** the daemon constructs one `NanoRuntimeRegistry` with the four real adapters and an explicit `bwrap-landlock` fallback. Productive agent start, restart, config apply, shift transition, restore replacement, operator despawn, shutdown, pause/resume, restart re-suspend, and control-plane suspension use the adapter selected by `[runtime].nano_runtime`; the selected adapter owns its process and resource lifecycle, while daemon cgroup/eBPF state remains observational. The merged idempotent per-workload stop contract (#698) fences every handle by incarnation and retains failed cleanup for an exact retry. Stop captures cgroup identity before adapter cleanup and removes eBPF registration only after success. bwrap spawn retains a recoverable transaction across marker, sandbox setup, and process start. Adapter control precedes the logical status event under a serialized ownership barrier; a typed unsupported or failed action leaves logical state unchanged. World snapshot schema v4 stores adapter snapshots and restore rejects incomplete or mismatched runtime sets. Runtime health and reconcile apply runtime-specific resource expectations instead of treating every adapter as bwrap. WASM requires an explicit component path. A microVM image must implement the versioned guest workload contract in [microvm-guest-workload-contract.md](microvm-guest-workload-contract.md); registering the adapter alone does not prove such an image. Snapshot semantics remain runtime-specific; local ECS-native migration (#413) remains separately available through `POST /operator/migrate`, while cross-node migration is out of scope. |
