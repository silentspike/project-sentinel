...
| Runtime contract | A compliant runtime implements seven operations: `spawn`, `exec`, `snapshot`, `restore`, `migrate`, `health`, and `isolate`. Initial runtime families are `ecs-native`, `wasm-wasmtime`, `bwrap-landlock`, and `microvm` (adapter + conformance realized; productive daemon-spawn integration tracked in #472 — see Realized row). |
