...
| Realized | **microVM adapter + conformance harness realized (#417), explicit stop lifecycle added (#698):** `crates/sentinel-microvm` implements the eight-operation `NanoRuntime` contract over Firecracker (KVM), including idempotent per-workload stop, and passes the shared conformance harness (#408; live-verified green on the Deploy-VM for the prior seven-operation contract). Snapshot semantics `MicrovmMemory` (stable metadata + deterministic Firecracker mem/state paths per `workload_id`). **Like all four NanoRuntime adapters (ecs-native, wasm-wasmtime, bwrap-landlock, microvm), this is NOT yet wired into the productive daemon spawn path** — `spawn_agent_full` uses `SandboxEnforcer`/`RuntimeOrchestrator` directly, not the `NanoRuntimeRegistry` (CRI); the adapters are reachable only via the registry in tests and `bin/nano_runtime_bench.rs`. Productive registry integration is tracked in **#472**. Separately delivered and live-verified: local live-migration as an observable first-class operation (#413, ECS-native, `POST /operator/migrate` + `MigrationCompleted` event); cross-node migration remains out of scope (Multi-Node-gated). |
...
| Revisit when | microVM support moves from future runtime family to implemented adapter; the adapters move from registry-only (tests/bench) into the productive daemon spawn path (tracked in #472); cross-node migration becomes a product requirement; or conformance evidence shows the eight-operation contract is too weak or too broad for real workloads |
