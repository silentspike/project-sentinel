...
| Realized | **Source candidate (#472; live verification pending):** the daemon constructs one `NanoRuntimeRegistry` with ECS-native, bwrap+Landlock, and optional WASM adapters plus an explicit `bwrap-landlock` fallback. Productive agent start, restart, staged config replacement, shift transition, restore replacement, operator despawn, shutdown, pause/resume, restart re-suspend, and control-plane suspension use the adapter selected by `[runtime].nano_runtime` through a private typed lifecycle owner; the selected adapter owns its process and resource lifecycle, while daemon cgroup/eBPF state remains observational. The merged idempotent per-workload stop contract (#698) fences every handle by incarnation and retains failed cleanup for an exact retry. Runtime health observes the exact adapter handle, health, resources, and logical status and fails closed on missing or mismatched incarnations; adapter `Degraded` is accepted only for an intentionally `Suspended` runtime. Config Apply uses one versioned SQLite decision saga with an idempotent filesystem participant, atomically records the completion event/outbox with the forward decision, and restores or forward-finalizes exact world/store/projection/runtime state under a startup fence. Default bwrap snapshots reproduce a fresh runtime incarnation from its bound workload specification; this is not process-memory, CRIU, or filesystem-content capture. Only the optional bwrap CAS filesystem-manifest extension is default-off until #548 supplies durable retained ownership, GC-safe new-before-old pin transfer, and complete retention/delete/restore/failed-walk behavior. WASM requires an explicit component path. The microVM adapter crate and historical KVM evidence remain available, but production selection is rejected until #775 implements the versioned guest launcher, workload-bound readiness attestation, and durable crash-recovery marker/reconcile contract after #687. #472 acceptance is explicitly limited to the three production adapters and no production microVM claim is made. Snapshot semantics remain runtime-specific; local ECS-native migration (#413) remains separately available through `POST /operator/migrate`, while cross-node migration remains in #553/#554 and is out of scope here. |
| Files | `docs/togaf-deviations-v22.md`, `docs/togaf-gap-v22.md`, `crates/sentinel-common/src/nano_runtime.rs`, `crates/sentinel-runtime/`, `crates/sentinel-wasm/`, `crates/sentinel-sandbox/`, `crates/sentinel-microvm/`, #397, #407, #408, #409, #410, #411, #413, #417, #472, #775 |
