...
| Frame codec | `crates/sentinel-cluster-control/src/envelope.rs`: `MAX_FRAME_BYTES = 1 << 20` (u32 **big-endian** length prefix, `to_be_bytes`/`from_be_bytes`), and **already-sketched** `RefQuery { block_ref }` / `PinQuery { block_ref }` request variants | Control RPCs (ref/pin queries) reuse as-is; **block bytes do not** (see below) |
| Gossip pattern (#495) | `crates/sentinel-common/src/membership.rs`: `Heartbeat` (`:38`), `MembershipView` (`:95`), `ingest` (`:115`, boot_id/incarnation ABA, receiver-monotonic clock); delivery uses `ControlRequest::MembershipHeartbeat` over the cert-pinned QUIC control plane | Block-map `HolderAdvertisement` mirrors `Heartbeat` 1:1 (see block-map-design doc) |
| Content identity (#500a) | `crates/sentinel-common/src/block_ref.rs`: `BlockRef` (`:130`), `BlockNamespace` (`:30`), `HashAlgorithm` (`:66`) | Canonical cross-node identifier (V7) -- **reuse**, do not redefine |
