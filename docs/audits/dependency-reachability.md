...
| --- | ---: | --- |
| `prune-features` | 8 | source and feature origin support a bounded removal or explicit feature set |
| `align-version` | 1 | dev-only Criterion pair can converge on the workspace version |
| `leave` | 2 | #632 compilation or release-tree proof disproved the proposed tracing JSON and direct-only zstd prunes |
