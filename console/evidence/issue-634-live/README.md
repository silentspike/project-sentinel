...
| AC-3 | The registry schema is complete and its active list is empty because the real tree has no Cargo patch, replace, or source-replacement override. | `docs/dependency-patches.md`, real-tree check below |
| AC-4 | The real tree passes; seven checker tests include all four required negative classes; the checker is in the always-running lint job. | `scripts/check-patch-registry.py`, `scripts/tests/test_check_patch_registry.py`, `.github/workflows/ci.yml`, `negative-tests.md` |
...
The four direct Git declarations point to the official Aya upstream and are not
patch/fork override rows:
...
test_ordinary_git_dependency_is_not_a_patch_or_fork ... ok
